...
class HomeController extends Controller
{
    ...
    public function updateIntroduction(Request $request){
        ...
        $user->introduction = $req['introduction1'];
        $user->save();
    ...
    public function updateInformation(Request $request)
    {
        ...
        $req = Purify::clean($request->except('aadhar','pan','image', '_token', '_method'));
		...
		if ($request->has('aadhar')) {
            $purifiedData['aadhar'] = $request->aadhar;
        }
		
		if ($request->has('pan')) {
            $purifiedData['pan'] = $request->pan;
        }
        ...
        $rules = [
			...
			'aadhar' => 'sometimes|max:3072|mimes:jpg,jpeg,png',
			'pan' => 'sometimes|max:3072|mimes:jpg,jpeg,png',
            'language_id' => Rule::in($languages),
        ];
        $message = [
			...
			'aadhar.required' => 'aadhar field is required',
            'aadhar.mimes' => 'This aadhar must be a file of type: jpg, jpeg, png.',
            'aadhar.max' => 'This aadhar may not be greater than :max kilobytes.',
			'pan.required' => 'pan field is required',
            'pan.mimes' => 'This pan must be a file of type: jpg, jpeg, png.',
            'pan.max' => 'This pan may not be greater than :max kilobytes.',
        ];
		...
		if($req['marital_status'] == 5)
			$req['no_of_children'] = 0;
		...
		if ($request->hasFile('aadhar')) {
            try {
                $user->aadhar = $this->uploadImage($request->aadhar, config('location.kyc.path'), config('location.user.size'), $old_image);
            } catch (\Exception $exp) {
                return back()->with('error', 'aadhar could not be uploaded.');
            }
        }
		
		if ($request->hasFile('pan')) {
            try {
                $user->pan = $this->uploadImage($request->pan, config('location.kyc.path'), config('location.user.size'), $old_image);
            } catch (\Exception $exp) {
                return back()->with('error', 'pan could not be uploaded.');
            }
        }
    ...
    public function physicalAttributes(Request $request)
    {
        ...
        $req = Purify::clean($request->except('bloodGroup','_token', '_method'));
    ...
    public function familyInformation(Request $request){
        ...
        $message = [
            ...
            'brother_married.required' => 'Married Brothers field is required',
            'sister_married.required' => 'Married Sisters field is required',
            'sibling_position.required' => 'Sibling Position field is required',
            'family_income.required' => 'Family Income field is required',
        ];
        ...
        $user->brother_married = $req['brother_married'] ?? "None";
        $user->sister_no = $req['sister_no'] ?? "None";
        $user->sibling_position = $req['sibling_position'];
        $user->family_income = $req['family_income'];
    ...
    public function partnerExpectation(Request $request){
		...
		if($user->gender == 'Male')
			...
			$req['partner_gender'] = 'Male';


		if(!isSet($req['partner_children_acceptancy']))
			$req['partner_children_acceptancy'] = 'No';
        ...
        $rules = [
            ...
            'partner_children_acceptancy' => 'nullable',
