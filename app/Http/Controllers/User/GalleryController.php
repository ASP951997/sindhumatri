...
class GalleryController extends Controller
{
	...
	public function galleryProfile($id){
        ...
        $galleryImageDelete = config('location.gallery.path').'thumb_'.$gallery->image;
		//exit;
		
        if(File::exists($galleryImageDelete)){
            File::copy($galleryImageDelete, config('location.user.path').$gallery->image);
			
			$this->user->image = $gallery->image;
			$this->user->save();
        }else{
			return back()->with('error', 'Image not found in request.');
		}
