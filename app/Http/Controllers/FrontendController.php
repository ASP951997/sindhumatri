...
class FrontendController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['plans'] = Plan::with('details')->where('status', 1)->orderBy(Plan::raw('CAST(price AS UNSIGNED)'), 'asc')->get();
        if(Auth::user()){
	...
	function convertHeightToInches($height) {
		...
		preg_match('/(\d+)ft\s*(\d*)in/', $height, $matches);
    ...
    public function searchMember(Request $request)
    {
		...
		//$search['max_height'] = "5ft 9in";
		//$search['min_height'] = "5ft 0in";
		//print_r($search);
		//echo $search['min_height']."-".$search['max_height'];
		//exit;
        if (Auth::check()) {
            $user = Auth::user();
            $ignoreList = collect([]);
        ...
        return $query->whereRaw('
            (SUBSTRING_INDEX(height, "ft", 1) * 12 + SUBSTRING_INDEX(SUBSTRING_INDEX(height, " ", -1), "in", 1)) <= ?', [$maxHeightInches]);
    })
    ->when(isset($search['min_height']), function ($query) use ($search) {
        $minHeightInches =  $this->convertHeightToInches($search['min_height']);
        return $query->whereRaw('
