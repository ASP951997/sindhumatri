        ...
        <div class="accordion-body">
            <form action="{{ route('user.partnerExpectation')}}" method="post" enctype="multipart/form-data">
                ...
                <div class="row g-3 g-md-4">
                        ...
                        <select class="form-select" name="partner_min_height" aria-label="partner_min_height">
							...
							@for ($ft = 4; $ft <= 7; $ft++)
								@for ($in = 0; $in < 12; $in++)
									...
									@endphp
									<option value="{{ $ft . 'ft ' . $in . 'in' }}" {{ $user->partner_min_height == ($ft . 'ft ' . $in . 'in') ? 'selected' : '' }}>
										{{ $heightString }}
									</option>
								@endfor
							@endfor
                    ...
                    <div class="col-md-6 form-group">
                        <label for="partner_max_weight">@lang('Max Height (In Feet)')</label> <span
                            class="text-danger">*</span>
                        <!-- <input
                            type="text"
                            class="form-control"
							...
							@for ($ft = 4; $ft <= 7; $ft++)
								@for ($in = 0; $in < 12; $in++)
									...
									@php
										...
										$heightString = $ft . 'ft ' . $in . 'in (' . $heightInCm . ' cm)';
									@endphp
									<option value="{{ $ft . 'ft ' . $in . 'in' }}" {{ $user->partner_max_weight == ($ft . 'ft ' . $in . 'in') ? 'selected' : '' }}>
										{{ $heightString }}
									</option>
								@endfor
							@endfor
                       ...
                       <!-- <select class="form-select" name="partner_max_weight" aria-label="partner_max_weight">
                            <option value="" disabled selected>@lang('Select Weight')</option>
                            @for ($ft = 40; $ft <= 100; $ft++)
									
                                    <option value="{{ $ft . ' kg' }}" {{ $user->partner_max_weight == ($ft . ' kg' ) ? 'selected' : '' }} >
                                        {{ $ft . ' kg' }}
                                    </option>
                            @endfor
                        </select> -->
                    ...
                    <div class="col-md-6 form-group">
                      ...
                      @if($user->partner_marital_status != '5')  
                    ...
                    <div class="col-md-6 form-group">
                        ...
                        >
                            <option value="" selected disabled>@lang('Select One')</option>
                            @foreach(config('languages')['langCodeWithoutFlagSindhi'] as $key => $item)
                                <option value="{{$item}}"
					...
					<div class="col-md-6 form-group">
                        ...
                        <select class="form-control" name="partner_education" aria-label="partner_education" data-container="body">  
							...
							<option value="Below 10th" {{ (old('degree') ?? $user->degree) == 'Below 10th' ? 'selected' : '' }}>Below 10th</option>
                            <option value="High School (10th)" {{ $user->partner_education == 'High School (10th)' ? 'selected' : '' }}>High School (10th)</option>
                            <option value="Intermediate (12th)" {{  $user->partner_education == 'Intermediate (12th)' ? 'selected' : '' }}>Intermediate (12th)</option>
                            <option value="Diploma" {{  $user->partner_education == 'Diploma' ? 'selected' : '' }}>Diploma</option>
                            <option value="Bachelor's Degree" {{  $user->partner_education == "Bachelor's Degree" ? 'selected' : '' }}>Bachelor's Degree</option>
                            <option value="Master's Degree" {{ $user->partner_education == "Master's Degree" ? 'selected' : '' }}>Master's Degree</option>
                            <option value="Doctorate (PhD)" {{ $user->partner_education == "Doctorate (PhD)" ? 'selected' : '' }}>Doctorate (PhD)</option>
                            <option value="Post-Doctorate" {{ $user->partner_education == "Post-Doctorate" ? 'selected' : '' }}>Post-Doctorate</option>
                        </select>
                        ...
                        >
                            ...
                            <option
                                value="No" {{old('partner_smoking_acceptancy', $user->partner_smoking_acceptancy == 'No') ? 'selected' : ''}}>@lang('No')</option>
								 <option
                                value="Occasionally" {{old('partner_smoking_acceptancy', $user->partner_smoking_acceptancy == 'Occasionally') ? 'selected' : ''}}>@lang('Occasionally')</option>
                            <option
                    ...
                    <div class="col-md-6 form-group">
                        ...
                        >
                            ...
                            <option
								...
								<option
                                value="Occasionally" {{old('partner_drinking_acceptancy', $user->partner_drinking_acceptancy == 'Occasionally') ? 'selected' : ''}}>@lang('Occasionally')</option>
                            <option
                    ...
                    <div class="col-md-6 form-group">
                        ...
                        >
                            <option value="" selected disabled>@lang('Select One')</option>
                            <option value="Veg" {{old('partner_dieting_acceptancy', $user->partner_dieting_acceptancy == 'Veg') ? 'selected' : ''}}>@lang('Veg')</option>
                            <option value="Non-Veg" {{old('partner_dieting_acceptancy', $user->partner_dieting_acceptancy == 'Non-Veg') ? 'selected' : ''}}>@lang('Non-Veg')</option>
                            <option value="Eggitarian" {{old('partner_dieting_acceptancy', $user->partner_dieting_acceptancy == 'Eggitarian') ? 'selected' : ''}}>@lang('Eggitarian')</option>
                            <option value="Occasionally non-veg" {{old('partner_dieting_acceptancy', $user->partner_dieting_acceptancy == 'Occasionally non-veg') ? 'selected' : ''}}>@lang('Occasionally non-veg')</option>
                        </select>
                    ...
                    <div class="col-md-6 form-group">
                        ...
                        >
                            ...
                            @foreach($bodyType as $data)
                                <option value="{{$data->body_types_id}}" {{($user->partner_body_type == $data->body_types_id) ? 'selected' : ''}}>@lang($data->name)</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6 form-group">
                        ...
                        >
                            ...
                            @foreach($complexion as $data)
                                <option value="{{$data->complexion_id}}" {{($user->partner_complexion == $data->complexion_id) ? 'selected' : ''}}>@lang($data->name)</option>
                            @endforeach
                        </select>
        ...
        function getAllPartnerCaste(idGetReligion = null, getSelectedCaste = null) {
            $.ajax({
                ...
                success: function (result) {
                    $('#partner_caste').html('<option value="" disabled>@lang("Select Caste")</option>');
                    ...
                    $('#partner_sub_caste').html('<option value="">@lang("Select Sub-Caste")</option>');
                }
            });
        }
